...
class ConfigTest < Minitest::Test
  def test_rejects_empty_repos
    path = write_config("github" => { "repos" => [] })
  ...
  def test_requires_chat_in_allowlist
    ...
    err = assert_raises(Prdigest::ConfigError) do
      Prdigest::Config.load(path, capability: :prose_delivery)
    end
    assert_match(/allowlist/, err.message)
  end

  def test_loads_valid_facts_config
    path = write_config(
  ...
  def test_delivery_state_has_a_direct_default_and_can_be_overridden
    path = write_config(
      "github" => { "repos" => ["o/r"] }
    )
    assert_equal File.expand_path("~/.local/share/prdigest/deliveries"),
                 Prdigest::Config.load(path).delivery_state_path
  ...
  def test_validates_timezone_and_repository_shape
    base = { "github" => { "repos" => ["owner/repo"] } }
  ...
  def test_facts_capability_still_validates_timezone_and_repositories
    configurations = [
      {
  ...
  def test_default_load_capability_is_facts_only
    path = write_config(
      "timezone" => "UTC",
      "github" => { "repos" => ["owner/repo"] }
    )

    assert_equal ["owner/repo"], Prdigest::Config.load(path).repos
  end
  ...
  def test_facts_does_not_validate_unselected_prose_configuration
